...
[중요: 출력 언어]
- 모든 출력 필드(title, topic_summary, slide_outline의 title/one_line, key_terms, domain_hints)는
  항상 자연스러운 한국어로 작성합니다.
- 슬라이드 자체가 중국어·영어·일본어 등 다른 언어로 쓰여 있어도 의미를 한국어로 번역하여 작성합니다.
- 고유명사·전문용어·약어는 한국어 옆에 원문 표기를 병기 (예: "성공 사례 (成功案例)", "API").
- 번역할 때 원문 텍스트를 같은 시퀀스로 반복하지 말고, 한 번 한국어로 옮긴 뒤 다음 항목으로 넘어가세요.
...
_PASS2_STATIC_BODY = """\
[중요: 출력 언어]
- 모든 출력 필드(title, markdown_body, image_caption, reasoning)는 항상 자연스러운 한국어로 작성합니다.
- 슬라이드 자체가 중국어·영어·일본어 등 다른 언어로 쓰여 있어도 의미를 한국어로 번역하여 작성합니다.
  (예: 슬라이드 제목이 "成功案例"이면 title은 "성공 사례 (成功案例)" 형태)
- 고유명사·전문용어·약어는 한국어 옆에 원문 표기를 병기. 영어 약어(API, BMI 등)는 그대로 사용.
- 번역할 때 같은 짧은 시퀀스를 반복 출력하지 마세요. 한 번 한국어로 옮긴 뒤 다음 단락으로 넘어가세요.
  같은 단어/구가 반복되어 보인다면 즉시 멈추고 다른 표현으로 넘어가야 합니다.
...
- 영어 약어/고유명사(API, BMI, PAPS, Tabular 등)를 한국어로 임의 번역 금지 (원문 표기 유지)
- 같은 짧은 시퀀스(단어, 자모, 음절)를 반복 출력 금지 — 반복 루프 감지 시 즉시 다른 표현으로 전환
